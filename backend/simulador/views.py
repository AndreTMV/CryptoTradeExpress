...
def guardar_precios_historicos(simulacion):
    ...
    for precio in precios:
        PrecioBitcoin.objects.create(simulacion=simulacion, fecha=precio['fecha'], precio=precio['precio'])
...
def obtener_transacciones(request):
    ...
    if user_id:
        try:
            ...
            return Response({'error': 'Usuario no encontrado'}, status=404)
        try:
            transacciones = Transaccion.objects.filter(user=user)
...
def obtener_precios_bitcoin(request):
    ...
    if user_id:
        ...
        try:
            simulacion = Simulacion.objects.get(user=user)
            precios = PrecioBitcoin.objects.filter(simulacion=simulacion).order_by('fecha')
...
#         try:
#             simulacion = Simulacion.objects.get(user=user)
#             simulacion.fecha += timedelta(days=31)
#             simulacion.save()
#             serializer = SimulacionSerializer(simulacion)
#             return Response(serializer.data, status=200)
#         except Simulacion.DoesNotExist:
...
def avanzar_simulacion(request):
    ...
    if user_id:
        ...
        try:
            ...
            precios_disponibles = PrecioBitcoin.objects.filter(simulacion=simulacion, fecha__gte=simulacion.fecha, fecha__lte=nueva_fecha).count()

            if precios_disponibles < 31:
                dias_a_predecir = 31 - precios_disponibles
                model, scaler, look_back = entrenar_modelo_lstm()
                predecir_precios(model, scaler, look_back, dias_a_predecir=dias_a_predecir, simulacion=simulacion)
...
def comprar_bitcoin(request):
    ...
    if user_id:
        ...
        try:
            ...
            precio_actual = PrecioBitcoin.objects.get(simulacion=simulacion, fecha=fecha_inicial)
            ...
            if simulacion.balance >= cantidad * precio:
                ...
                simulacion.save()
                Transaccion.objects.create(simulacion=simulacion, tipo='compra', cantidad=cantidad, precio=precio)
            else:
    ...
    if user_id:
        ...
        try:
            ...
            if simulacion.bitcoins >= cantidad:
                ...
                simulacion.fecha = fecha_inicial
                simulacion.save()
                Transaccion.objects.create(simulacion=simulacion, tipo='venta', cantidad=cantidad, precio=precio)
            else:
...
def actualizar_precios_futuros(request):
    ...
    if user_id:
        ...
        try:
            simulacion = Simulacion.objects.get(user=user)
            model, scaler, look_back = entrenar_modelo_lstm()
            predicciones = predecir_precios_bd(model, scaler, look_back, simulacion=simulacion)
            fecha = request.data.get('fecha')
            fecha_inicial = datetime.strptime(fecha, '%Y-%m-%d').date()
